...
typedef enum _IMV_EPixelType
{
	...
	// YUV420
	gvspPixelYUV420SP_NV12 = (IMV_GVSP_PIX_COLOR | IMV_GVSP_PIX_OCCUPY12BIT | 0x8001),
...
typedef struct _IMV_UsbInterfaceInfo
{
	...
	char portID[IMV_MAX_STRING_LENTH];					///< \~chinese  USB接口port			\~english USB interface port
	char chReserved[3][IMV_MAX_STRING_LENTH];			///< 保留							\~english Reserved field
...
#define IMV_MSG_EVENT_ID_FRAME_END					0x9007
#define IMV_MSG_EVENT_ID_TRIGGER_IGNORED			0x9008
#define IMV_MSG_EVENT_ID_FRAMEACTIVE_END			0x9009
#define IMV_MSG_EVENT_ID_FIRST_LINE					0x900A
#define IMV_MSG_EVENT_ID_LAST_LINE					0x900B
#define IMV_MSG_EVENT_ID_EXPOSURE_ACTICE_UP 		0x900C
#define IMV_MSG_EVENT_ID_FRAME_PERIOD_LINE_COUNT	0x900D
#define IMV_MSG_EVENT_ID_TRIGGER_RANDOM				0x900E
#define IMV_MSG_EVENT_ID_LINECAM_CUSTOM				0x900F
#define IMV_MSG_EVENT_ID_FRAMEACTIVE_START			0x9010
...
typedef struct _IMV_RecordFrameInfoParam
{
	...
	unsigned int			nPaddingX;						///< [IN]	\~chinese 图像宽填充(可忽略)			\~english Padding X(Ignorable)
	unsigned int			nPaddingY;						///< [IN]	\~chinese 图像高填充(可忽略)			\~english Padding Y(Ignorable)
	IMV_EPixelType			ePixelFormat;					///< [IN]	\~chinese 像素格式(可忽略)				\~english Pixel format(Ignorable)
...
typedef enum _IMV_ESaveFileType
{
	...
	typeSaveTiff = 3,                       ///< \~chinese TIFF原图像格式											\~english TIFF original image format
	typeSaveTiff_Extend = 4,				///< \~chinese TIFF有效位拉伸至16Bit图像格式							\~english TIFF Stretching effective bits to 16 bit image format
	typeSaveTiff_RGB = 5,					///< \~chinese TIFF(仅支持Bayer10\12\16)原图像格式						\~english TIFF (support Bayer10\12\16 only) Original image format
	typeSaveTiff_RGB_Extend = 6,			///< \~chinese TIFF(仅支持Bayer10\12\16)有效位拉伸至16Bit图像格式		\~english TIFF (support Bayer10\12\16 only) Stretching effective bits to 16 bit image format
...
/// \brief 保存图像到文件结构体
/// \~english
/// \brief Save image to file structure
...
typedef struct _IMV_SaveImageParam
{	
	unsigned int        nWidth;             ///< [IN]   \~chinese 图像宽								\~english Width
	unsigned int        nHeight;            ///< [IN]   \~chinese 图像高							    \~english Height
	IMV_EPixelType      ePixelFormat;		///< [IN]   \~chinese 输入数据的像素格式				    \~english Pixel format	
	...
	unsigned int        nSrcDataLen;        ///< [IN]	\~chinese 输入数据大小							\~english Input image length
	IMV_ESaveFileType	eImageType;			///< [IN]	\~chinese 输入保存图片格式						\~english Input image save format
	unsigned int		nQuality;			///< [IN]	\~chinese JPG编码质量(0-100]，其它格式无效		\~english JPG Encoding quality(0-100]，Other formats are invalid	
	IMV_EBayerDemosaic  eBayerDemosaic;     ///< [IN]	\~chinese 转换Bayer格式算法						\~english Alorithm used for Bayer demosaic	
	unsigned int		nDstBufSize;		///< [IN]	\~chinese 提供的输出缓冲区大小  				\~english Provided output buffer size
...
///brief 属性可见性
/// \~english
/// \brief Attribute visibility
typedef enum _IMV_EFeatureVisibility
{
	IMV_VISIBEGINNER,						///< \~chinese 初学者										\~english beginner
	IMV_VISIEXPERT,							///< \~chinese 专家											\~english expert
	IMV_VISIGURU,							///< \~chinese 骨灰级专家									\~english Ash level expert
	IMV_VISIINVISBLE,						///< \~chinese 不可见										\~english invisible
	IMV_VISIUNDEFINED = 99					///< \~chinese 未定义										\~english undefined
}IMV_EFeatureVisibility;
