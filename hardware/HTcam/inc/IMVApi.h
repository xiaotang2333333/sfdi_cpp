...
/// \param handle			[IN] Device handle
/// \param transmissionType [IN] Struct of transmission type
/// \return Success, return IMV_OK. Failure, return error code 
IMV_API int IMV_CALL IMV_GIGE_SetTransmissionType(IN IMV_HANDLE handle, IN IMV_TRANSMISSION_TYPE transmissionType);
...
/// \~chinese
/// \brief 从文件加载设备xml配置（IP，子网掩码，默认网关不导入)）
/// \param handle [IN] 设备句柄
/// \param pFullFileName [IN] 设备配置(xml)文件全名(含路径)，如：D:\\config.xml 或 D:\\config.mvcfg
/// \param pErrorList [OUT] 加载失败的属性名列表。存放加载失败的属性上限为IMV_MAX_ERROR_LIST_NUM。
...
/// \return 成功，返回IMV_OK；错误，返回错误码
/// \~english 
/// \brief load the configuration of the device (Support user-defined selection of whether to import camera network configurations, default not to import)
/// \param handle [IN] Device handle
/// \param pFullFileName [IN] The full path name of the property file(xml). e.g. D:\\config.xml or D:\\config.mvcfg
/// \param pErrorList [OUT] The list of load failed properties. The failed to load properties list up to IMV_MAX_ERROR_LIST_NUM.
/// \param isLoadNetConfig [in] Whether to load network related configurations
/// \return Success, return IMV_OK. Failure, return error code 
IMV_API int IMV_CALL IMV_LoadDeviceCfgEx(IN IMV_HANDLE handle, IN const char* pFullFileName, OUT IMV_ErrorList* pErrorList);
...
/// 只支持一个回调函数,且设备关闭后，注册会失效，打开设备后需重新注册
/// \~english
/// \brief Register call back function of message channel event(GigE Only).
/// \param handle [IN] Device handle
...
/// Only one call back function is supported.\n
/// Registration becomes invalid after the device is closed, , and need to re-register after the device is opened
IMV_API int IMV_CALL IMV_SubscribeMsgChannelArgEx(IN IMV_HANDLE handle, IN IMV_MsgChannelCallBackEx proc, IN void* pUser);
...
/// \param handle [IN] 设备句柄
/// \param pFeatureName [IN] 属性名
/// \param pFeatureVisibility [OUT] 属性可见性
/// \return 成功，返回IMV_OK；错误，返回错误码
/// \~english
/// \brief Get attribute visibility
/// \param handle [IN] Device handle
/// \param pFeatureName [IN] Feature name
/// \param pFeatureVisibility [OUT] Attribute visibility
/// \return Success, return IMV_OK. Failure, return error code 
IMV_API int IMV_CALL IMV_GetFeatureVisibility(IN IMV_HANDLE handle, IN const char* pFeatureName, OUT IMV_EFeatureVisibility* pFeatureVisibility);
...
/// \~chinese
/// \brief 保存图像,支持Bmp和Jpeg
/// \param handle [IN] 设备句柄
/// \param pstSaveImageParam [IN][OUT] 保存图片参数结构体
/// \return 成功，返回IMV_OK；错误，返回错误码
/// \remarks
/// JPEG格式最大支持宽高为65500
/// 通过该接口可以将图像数据转换为JPEG或BMP格式并放在指定内存中，客户可以直接将转换之后的数据保存成图片文件
/// \~english
/// \brief Save the image to file.
/// \param handle [IN] Device handle
/// \param pstSaveImageParam [IN][OUT] Save image parameter structure
/// \return Success, return IMV_OK. Failure, return error code
/// \remarks
/// JPEG format supports a maximum width and height of 65500
/// This interface can be used to convert image data into JPEG or BMP format and store it in the specified memory. Customers can directly save the converted data into image files
IMV_API int IMV_CALL IMV_SaveImage(IN IMV_HANDLE handle, IN IMV_SaveImageParam* pstSaveImageParam);
#ifdef __cplusplus
}
#endif 
